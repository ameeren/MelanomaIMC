...
```{r}
sce[,sce$celltype %in% c("exhausted Tcytotoxic", "Tcytotoxic")]$celltype <- "Tcytotoxic"
sce[,sce$celltype %in% c("CD134+ Tcell", "CD134- Tcell")]$celltype <- "Tcell"
```
...
celltype_color["CD38"] <- "goldenrod2"
celltype_color["HLA-DR"] <- "green1"
celltype_color["Macrophage"] <- "greenyellow"
celltype_color["Neutrophil"] <- "blue1"
celltype_color["Tcell"] <- "yellow"
celltype_color["Vasculature"] <- "red2"
...
cur_exprs <- cbind(cur_exprs, cur_sce[,c("celltype", "celltypes_layer1", "celltype_clustered")])
cur_exprs$cellID <- rownames(cur_exprs)

cur_exprs <- cur_exprs %>%
  reshape2::melt(id.vars = c("cellID", "celltype", "celltypes_layer1", "celltype_clustered"), 
...
b <- cur_exprs %>%
  filter(celltype == "Tcytotoxic" & marker %in% marker_list$Tcytotoxic) %>%
  ggplot(data=., aes(asinh, color = celltypes_layer1)) + 
  geom_density() +
  xlim(0,4) + 
  facet_wrap(~marker)
