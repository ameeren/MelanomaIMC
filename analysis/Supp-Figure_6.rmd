...
## run UMAP on stand-alone expressing tumor cells and control group

```{r Fig_6A_UMAP_standalone_tumor, dev=('pdf'), fig.width=11, fig.height=6}
...
a <- dittoDimPlot(cur_sce,
    ...
    guides(color = guide_legend("Chemokine", override.aes = list(alpha=1, size=5)))
...
```{r Fig_6D_heatmap, dev=('pdf'), fig.width=11, fig.height=9}
standalone_tumor <- data.frame(colData(sce_rna[, sce_rna$standalone_tumor == TRUE & sce_rna$chemokine == 1]))[,"cellID"]
standalone_neighbours <- dat_relation[dat_relation$cellID_first %in% standalone_tumor,]$cellID_second
...
dittoHeatmap(agr_sce[rowData(agr_sce)$good_marker,], assay = "asinh",
            annot.by = c("celltype", "tumor_neighbor"), 
            cluster_rows = TRUE,
