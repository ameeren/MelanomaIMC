...
1. checking how many cells contain multiple labels
...
```{r quality-control-1}
# 1. percentage of cells with multiple labels
sum(duplicated(names(labels)))/length(unique(names(labels))) * 100

# Select unique cells
dup <- names(labels)[duplicated(names(labels))]
labels <- labels[!(names(labels) %in% dup)]
length(unique(names(labels)))/length(labels)

# Store this vector in SCE object
label_vector <- rep("unlabelled", ncol(sce))
names(label_vector) <- colnames(sce)
...
as_tibble(colData(sce)) %>%
  group_by(layer_1_gated, SampleId) %>%
  summarise(number_cells = n()) %>%
  as.data.frame()

# Total cells per class and image
as_tibble(colData(sce)) %>%
  group_by(layer_1_gated, SampleId, ROI) %>%
  summarise(number_cells = n()) %>%
  as.data.frame()
```
...
agr_sce <- aggregateAcrossCells(lab_sce, ids = colData(lab_sce)[,c("SampleId", "layer_1_gated")], 
                                average = TRUE)
