...
SKCM_GSE123139 <- read_Data(data = "../Downloads/SKCM_GSE123139_expression.h5",
                      metadata = "../Downloads/SKCM_GSE123139_CellMetainfo_table.tsv",
                      name = "SKCM_GSE123139",
                      sorting = "immune")

SKCM_GSE72056 <- read_Data(data = "../Downloads/SKCM_GSE72056_expression.h5",
                      metadata = "../Downloads/SKCM_GSE72056_CellMetainfo_table.tsv",
                      name = "SKCM_GSE72056",
                      sorting = "all")

SKCM_GSE115978 <- read_Data(data = "../Downloads/SKCM_GSE115978_aPD1_expression.h5",
                      metadata = "../Downloads/SKCM_GSE115978_aPD1_CellMetainfo_table.tsv",
                      name = "SKCM_GSE115978",
                      sorting = "all")

SKCM_GSE120575 <- read_Data(data = "../Downloads/SKCM_GSE120575_aPD1aCTLA4_expression.h5",
                      metadata = "../Downloads/SKCM_GSE120575_aPD1aCTLA4_CellMetainfo_table.tsv",
                      name = "SKCM_GSE120575",
                      sorting = "immune")

SKCM_GSE139249 <- read_Data(data = "../Downloads/SKCM_GSE139249_expression.h5",
                      metadata = "../Downloads/SKCM_GSE139249_CellMetainfo_table.tsv",
                      name = "SKCM_GSE139249",
                      sorting = "NKcells")

HNSC_GSE139324 <- read_Data(data = "../Downloads/HNSC_GSE139324_expression.h5",
                      metadata = "../Downloads/HNSC_GSE139324_CellMetainfo_table.tsv",
                      name = "HNSC_GSE139324",
                      sorting = "all")

NSCLC_GSE131907 <- read_Data(data = "../Downloads/NSCLC_GSE131907_expression.h5",
                      metadata = "../Downloads/NSCLC_GSE131907_CellMetainfo_table.tsv",
                      name = "NSCLC_GSE131907",
                      sorting = "all")

PAAD_CRA001160 <- read_Data(data = "../Downloads/PAAD_CRA001160_expression.h5",
                      metadata = "../Downloads/PAAD_CRA001160_CellMetainfo_table.tsv",
                      name = "PAAD_CRA001160",
                      sorting = "all")

UVM_GSE139829 <- read_Data(data = "../Downloads/UVM_GSE139829_expression.h5",
                      metadata = "../Downloads/UVM_GSE139829_CellMetainfo_table.tsv",
                      name = "UVM_GSE139829",
                      sorting = "all")

# merge the datasets
sc_dat <- rbind(SKCM_GSE115978,SKCM_GSE120575,SKCM_GSE123139,SKCM_GSE139249,SKCM_GSE72056,HNSC_GSE139324,NSCLC_GSE131907,PAAD_CRA001160,UVM_GSE139829)
...
ggplot()+
  ...
  geom_point(data = plot_dat,aes(x=celltype,y=frac_of_chemokine_pos,col=as.factor(dataset)))+
...
```{r add significance to plot, fig.height=15,fig.width=15}
library(ggpubr)

ggplot()+
  geom_boxplot(data = plot_dat,aes(x=celltype,y=frac_of_chemokine_pos))+
  geom_point(data = plot_dat,aes(x=celltype,y=frac_of_chemokine_pos,col=as.factor(dataset)))+
  geom_point(data = imc_long,aes(x=celltype,y=frac_of_chemokine_pos),color="red",shape=18, size=5)+
...
```{r}
read_Data_unprocessed <- function(data,
                      metadata,
                      name,
                      sorting){
...
```{r}
HNSC <- read_Data_unprocessed(data = "../Downloads/HNSC_GSE139324_expression.h5",
                      metadata = "../Downloads/HNSC_GSE139324_CellMetainfo_table.tsv",
                      name = "HNSC_GSE139324",
                      sorting = "all")

doubl_vec <- HNSC %>%
  select(colnames(HNSC)[grepl("CCL2$|CCL4$|CCL8|CCL18|CCL19|CCL22|CXCL8|CXCL9|CXCL10|CXCL12|CXCL13",colnames(HNSC))]) %>%
  rowSums()

length(doubl_vec[which(doubl_vec %in% c(2:9))])/length(doubl_vec[doubl_vec == 1])
```
...
```{r}
NSCLC <- read_Data_unprocessed(data = "../Downloads/NSCLC_GSE131907_expression.h5",
                      metadata = "../Downloads/NSCLC_GSE131907_CellMetainfo_table.tsv",
                      name = "NSCLC_GSE131907",
                      sorting = "all")

doubl_vec <- NSCLC %>%
  select(colnames(NSCLC)[grepl("CCL2$|CCL4$|CCL8|CCL18|CCL19|CCL22|CXCL8|CXCL9|CXCL10|CXCL12|CXCL13",colnames(NSCLC))]) %>%
  rowSums()

NSCLC[which(doubl_vec %in% c(2:9)),]

length(doubl_vec[which(doubl_vec %in% c(2:9))])/length(doubl_vec[doubl_vec == 1])
```

```{r}
UVM <- read_Data_unprocessed(data = "../Downloads/UVM_GSE139829_expression.h5",
                      metadata = "../Downloads/UVM_GSE139829_CellMetainfo_table.tsv",
                      name = "UVM_GSE139829",
                      sorting = "all")

doubl_vec <- UVM %>%
  select(colnames(UVM)[grepl("CCL2$|CCL4$|CCL8|CCL18|CCL19|CCL22|CXCL8|CXCL9|CXCL10|CXCL12|CXCL13",colnames(UVM))]) %>%
  rowSums()
...
```{r}
MEL <- read_Data_unprocessed(data = "../Downloads/SKCM_GSE72056_expression.h5",
                      metadata = "../Downloads/SKCM_GSE72056_CellMetainfo_table.tsv",
                      name = "SKCM_GSE72056",
                      sorting = "all")

doubl_vec <- MEL %>%
  select(colnames(MEL)[grepl("CCL2$|CCL4$|CCL8|CCL18|CCL19|CCL22|CXCL8|CXCL9|CXCL10|CXCL12|CXCL13",colnames(MEL))]) %>%
  rowSums()
