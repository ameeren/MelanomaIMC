...
# expressor fractions image
fraction <- cur_dt %>%
  select(ImageNumber, expressor) %>%
  filter(expressor != "NA") %>%
  group_by(ImageNumber, expressor) %>%
  summarise(n=n()) %>%
  group_by(expressor) %>%
  mutate(fraction = n / sum(n)) %>%
  reshape2::dcast(expressor ~ ImageNumber, value.var = "fraction", fill = 0) %>%
  filter(expressor %in% targets)

# change row-order to match targets and remove expressor column (only keep fractions)
fraction <- fraction[match(targets,fraction$expressor),-1]

# sort row-wise
#fraction <- apply(fraction,1,sort, decreasing=TRUE)

# max fraction coming from one image per expressor
max_fraction <- paste0(round(apply(fraction,1,max),3)*100, "%")
median_fraction <- paste0(round(apply(fraction,1,median),3)*100, "%")
...
ht = UpSet(m, 
    ...
    left_annotation = rowAnnotation(
        ...
        annotation_name_gp = gpar(fontsize=20)),
    bottom_annotation = HeatmapAnnotation(foo = anno_barplot(fraction, gp = gpar(fill = 2:161),
                                                             bar_width = 1, height = unit(2, "cm")),
                                          Max = anno_text(max_fraction,
                                                          just = "center", location = 0.5,
                                                           rot = 0,
                                                           gp = gpar(fontsize=8)),
                                          Median = anno_text(median_fraction,
                                                             just = "center", location = 0.5,
                                                             rot = 0,
                                                             gp = gpar(fontsize=8))),
...
```{r Fig_2D_pearson_correlation_chemokines_prot_data, fig.width=9, fig.height=9, dev=c('pdf')}
