...
# replace order of chemokines to match order without control samples (ATTENTION! HARD CODED)
color_chemo <- replace(color_chemo, c("CXCL8", "CCL18"), color_chemo[c("CCL18", "CXCL8")])
metadata(sce_rna)$colour_vectors$chemokine_combinations_no_control <- color_chemo
```
...
```{r Fig_2A_upset_plot, message=F, fig.width=14, fig.height=13, dev=c('pdf')}
                                          ...
                                          "Median Number of Chemokine- \nProducing Cells per Location" = 
                                            anno_barplot(location[,-1],
                                                         ...
                                                         gp = gpar(fill = grDevices::hcl.colors(ncol(location[,-1]), 
                                                                                                palette = "Dark 2"))),
                                          height = unit(5, "cm"),
                                          col = list("Combination Coloring" = metadata(sce_rna)$colour_vectors$chemokine_combinations_no_control)),
...
```{r Fig_2B_normalized_expression_met_location, fig.width=14, fig.height=9, dev=c('pdf')}
...
```{r Fig_2D_alluvial_tumor_cells, fig.width=13, fig.height=8, dev=c('pdf')}
...
```{r normalized_counts_chemokines_tumor, fig.width=10, fig.height=8, dev=c('pdf')}
...
```{r cxcl10_tumor_ki67_score, fig.width=10, fig.height=8, dev=c('pdf')}
