...
## Interaction between Dysfunctional CD8 and Tumor Cells V1 

```{r Fig_4D_corr_interaction_and_dysfunction, dev=("pdf"), fig.width=10.7, fig.height=4.2}
...
## Interaction between Dysfunctional CD8 and Tumor Cells V2

```{r Fig_4D_corr_interaction_and_dysfunctionX, dev=("pdf"), fig.width=10.7, fig.height=4.2, eval=F}
# number of interactions CD8+ T cell and tumor per image
dat_relation_sub <- dat_relation_rna[dat_relation_rna$celltype_first %in% c("exhausted Tcytotoxic", "Tcytotoxic") & 
                                   dat_relation_rna$celltype_second == "Tumor"]
...
## Number of CD8 per Tumor Cell

```{r Fig_4D_corr_interaction_and_dysfunction_normalizedTumor, dev=("pdf"), fig.width=8.4, fig.height=3.5, eval=F}
# number of interactions CD8+ T cell and tumor per image
dat_relation_sub <- dat_relation_rna[dat_relation_rna$celltype_first %in% c("exhausted Tcytotoxic", "Tcytotoxic") & 
                                   dat_relation_rna$celltype_second == "Tumor"]
...
## Number of Tumor per CD8+

```{r Fig_4D_corr_interaction_and_dysfunction_normalizedCD8, dev=("pdf"), fig.width=8.4, fig.height=3.5, eval=F}
# number of interactions CD8+ T cell and tumor per image
dat_relation_sub <- dat_relation_rna[dat_relation_rna$celltype_first %in% c("exhausted Tcytotoxic", "Tcytotoxic") & 
                                   dat_relation_rna$celltype_second == "Tumor"]
...
```{r Fig_4E_interaction_cytotoxic_tumor_boxplots, fig.width=8.8, fig.height=4, dev=("pdf")}
...
cur_dat_patient <- cur_dat %>%
  group_by(PatientID, cluster_number) %>%
  mutate(n_per_mm2_total = sum(n_per_mm2)) %>%
  distinct(PatientID, cluster_number, .keep_all = T)

ggplot(cur_dat_patient, aes(x=cluster_number, y=log10(n_per_mm2_total+1))) + 
...
```{r, Fig_4H_responder_nonresponder_tumor_perPatient, fig.height=4, fig.width=8.8, dev=("pdf")}
im_size <- as.data.frame(cbind(image_mat_prot$Metadata_Description, (image_mat_prot$Height_cellmask * image_mat_prot$Width_cellmask)/1000000))
names(im_size) <- c("Description", "mm2")
im_size$mm2 <- as.numeric(im_size$mm2)
im_size[im_size$Description %in% c("G1", "G1 - split"), ]$mm2 <- mean(im_size[im_size$Description %in% c("G1", "G1 - split"), ]$mm2)
...
cur_dat_patient <- cur_dat %>%
  ...
  mutate(mm2_total = sum(mm2)) %>%
  distinct(PatientID, cluster_number, .keep_all = T) %>%
  mutate(n_per_mm2_total = n_total / mm2_total)

stat.test <- cur_dat_patient %>%
  group_by(cluster_number) %>%
  mutate(y_log10 = log10(n_per_mm2_total+1)) %>%
  wilcox_test(data = ., y_log10 ~ Status_at_3m) %>%
  adjust_pvalue(method = "BH") %>%
  add_significance("p.adj",cutpoints = c(0, 1e-04, 0.001, 0.01, 0.1, 1)) %>%
  add_x_position(x = "cluster_number", dodge = 0.8)
