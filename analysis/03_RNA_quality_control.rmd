...
sapply(list.files("/Volumes/thoch/Git/ZTMA256_RNA_data/code/helper_functions", full.names = TRUE), source)
...
plotCellCounts(sce, colour_by = "Location", split_by = "ImageNumber")
```
...
rowData(sce)$good_marker <- ! grepl( "DNA|Histone|Vimentin|PD1|PDL1|CCR2",rownames(sce))
```
...
# TSNE
start = Sys.time()
sce <- runTSNE(sce, exprs_values = "scaled_asinh", 
...
# UMAP
start = Sys.time()
sce <- runUMAP(sce, exprs_values = "scaled_asinh", 
                     subset_row = rowData(sce)$good_marker)
end = Sys.time()
print(end-start)
```

## Subset SCE for UMAP/tSNE visualization
...
## Visualize features on tSNE and UMAP
Next, we will visualize different quality features on these representations.

```{r TSNE_clinical_features, fig.width=35, fig.height=25, message=FALSE}
# Select plots in list
p.list <- list()

# 
p.list$ImageNumber <- dittoDimPlot(cur_sce, var = "ImageNumber", reduction.use = "TSNE", size = 0.5, legend.show = FALSE) 
p.list$Mutation <- dittoDimPlot(cur_sce, var = "Mutation", reduction.use = "TSNE", size = 0.5)
p.list$Cancer_Stage <- dittoDimPlot(cur_sce, var = "Cancer_Stage", reduction.use = "TSNE", size = 0.5)
p.list$relapse <- dittoDimPlot(cur_sce, var = "relapse", reduction.use = "TSNE", size = 0.5)
p.list$Location <- dittoDimPlot(cur_sce, var = "Location", reduction.use = "TSNE", size = 0.5)
p.list$TissueType <- dittoDimPlot(cur_sce, var = "TissueType", reduction.use = "TSNE", size = 0.5)
p.list$MM_location_simplified <- dittoDimPlot(cur_sce, var = "MM_location_simplified", reduction.use = "TSNE", size = 0.5)
p.list$treatment_group_before_surgery <- dittoDimPlot(cur_sce, var = "treatment_group_before_surgery", reduction.use = "TSNE", size = 0.5)
...
## Marker Expression on TSNE

```{r asinh_TSNE_marker, fig.width=35, fig.height=25, message=FALSE}
p.list <- list()
for(i in rownames(sce)[rowData(cur_sce)$good_marker]){
  p.list[[i]] <- plotTSNE(cur_sce, colour_by = i, by_exprs_values = "asinh")
}
...
```{r scaled_asinh_TSNE_marker, fig.width=35, fig.height=25, message=FALSE}
p.list <- list()
for(i in rownames(sce)[rowData(cur_sce)$good_marker]){
  p.list[[i]] <- plotTSNE(cur_sce, colour_by = i, by_exprs_values = "scaled_asinh") 
}
