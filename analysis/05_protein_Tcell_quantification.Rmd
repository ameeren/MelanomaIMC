...
cellcount <- cellcount %>%
  mutate(Tcytotoxic_frac =n/total) %>%
  arrange(Tcytotoxic_frac)

# the lowest third of cytotoxic T cell fraction BlockIDs get grouped
cellcount$T_frac_score <- ""
cellcount[1:30,]$T_frac_score <- "low"
cellcount[31:61,]$T_frac_score <- "med"
cellcount[62:92,]$T_frac_score <- "high"
...
# the lowest third of cytotoxic T cell fraction BlockIDs get grouped
cellcount$T_frac_score <- ""
cellcount[1:23,]$T_frac_score <- "low"
cellcount[24:46,]$T_frac_score <- "med"
cellcount[47:70,]$T_frac_score <- "high"
...
p4 <- plotCellCounts(sce[,! sce$Location == "CTRL"],colour_by = "celltype",split_by = "T_frac_score_per_ImageNumber",proportion = FALSE)+
  scale_fill_manual(values = colour_vector)
...
there are 41905 cytotoxic T cells in inflamed images, 13141 in excluded images and 648 in deserted images.
```{r number of Tcytotoxic per infiltration class}
table(sce[,sce$celltype == "Tcytotoxic"]$infiltration)
```

## check the T cell marker profile across the infiltration categories


```{r Tcytotoxic marker profile for infiltration status, fig.height=15, fig.width=15} 
mean_sce <- calculateSummary(sce,split_by = c("celltype","celltype_clustered","E_I_D","infiltration"),exprs_values = "counts")
...
colnames(mean_sce) <- as.character(paste(mean_sce$celltype,mean_sce$celltype_clustered,mean_sce$E_I_D,mean_sce$infiltration,sep="__"))
...
cur_df$E_I_D <- sapply(cur_df$sample, function(x){strsplit(x,split = "__")[[1]][3]})
cur_df$infiltration <- sapply(cur_df$sample, function(x){strsplit(x,split = "__")[[1]][4]})

# mean expression of markers per cluster colored by Batch 

ggplot(cur_df[cur_df$celltype == "Tcytotoxic",],aes(x=marker,y = value,fill= factor(infiltration)))+
  #facet_wrap(~E_I_D)+
  geom_boxplot(outlier.shape = NA) +
  #geom_jitter(aes(x=marker,y =value,color=as.factor(E_I_D)),position = position_jitter( width = .1))+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
## check the T cell marker profile across the E_I_D categories

```{r Tcytotoxic marker profile for E_I_D status, fig.height=15, fig.width=15} 
ggplot(cur_df[cur_df$celltype == "Tcytotoxic",],aes(x=marker,y = value, fill = factor(E_I_D)))+
  #facet_wrap(~E_I_D)+
  geom_boxplot(outlier.shape = NA) +
  #geom_jitter(aes(x=marker,y =value,color=as.factor(infiltration)),position = position_jitter( width = .1))+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
```{r heatmap celltype marker for infiltration status, fig.height=15, fig.width=15}
plot_targets <- rownames(sce)
plot_targets <- plot_targets[! plot_targets %in% c("DNA1","DNA2","HistoneH3")]

mean_sce <- calculateSummary(sce,split_by = c("celltype","infiltration"),exprs_values = "counts")
...
plotHeatmap(mean_sce, exprs_values = "arcsinh", features = plot_targets,
            colour_columns_by = c("celltype","infiltration"), 
            color = viridis(100))
```


```{r heatmap celltype marker for infiltration status asinh scaled, fig.height=15, fig.width=15}
plotHeatmap(mean_sce, exprs_values = "arcsinh_scaled",  features = plot_targets,
            colour_columns_by = c("celltype","infiltration"), zlim = c(-4,4),
...
```{r heatmap Tcytotoxic for infiltration status asinh, fig.height=15, fig.width=15}
plot_targets <- rownames(sce)
plot_targets <- plot_targets[! plot_targets %in% c("DNA1","DNA2","HistoneH3")]

mean_sce <- calculateSummary(sce[,sce$celltype == "Tcytotoxic"],split_by = c("celltype_clustered","infiltration"),exprs_values = "counts")

assay(mean_sce, "arcsinh") <- asinh(assay(mean_sce, "meanCounts")) 
assay(mean_sce, "arcsinh_scaled") <- t(scale(t(asinh(assay(mean_sce, "meanCounts")))))

plotHeatmap(mean_sce, exprs_values = "arcsinh", features = plot_targets,
            colour_columns_by = c("celltype_clustered","infiltration"), 
            color = viridis(100))
```


```{r heatmap Tcytotoxic for infiltration status asinh_scaled, fig.height=15, fig.width=15}
plotHeatmap(mean_sce, exprs_values = "arcsinh_scaled",  features = plot_targets,
            colour_columns_by = c("celltype_clustered","infiltration"), zlim = c(-4,4),
            color = colorRampPalette(c("dark blue", "white", "dark red"))(100))
```
