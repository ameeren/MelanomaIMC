...
```{r}
library(tidyverse)
cor_dat <- matrix(nrow = 32, ncol = 25)
...
pdf("/media/dan/Data/Data/Melanoma_HotCold_paper/CXCL13_per_patient_RNA_protein_correlation.pdf")
ggplot(all_dat, aes(x=prot_CXCL13, y=rna_frac)) + 
  geom_point(size=3) + 
  geom_smooth(method="lm") +
  stat_cor(method = "spearman") + 
...
## per image correlation for exhausted clusters 2,3,4,10

```{r}
ex_clusters <- c(2,3,4,10)
