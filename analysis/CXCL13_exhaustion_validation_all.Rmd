...
```
cut off generally at: 2 asinh counts
for E2, F2, D4: 2.5

CXCL13 the cut off can generally be 2 but again:
for E2, F2 and D4 should be: 2.75

```{r, fig.height=5, fig.width=5}
for (i  in unique(sce$Description)) {
  p <-dittoScatterPlot(sce[,which(sce$Description == i )],x.var = "CXCL13",y.var = "PD-1",assay.x = "asinh",assay.y = "asinh", main=i,)
  plot(p)
}
```

general cut off for CD3: 1
for samples E2 and F2: 1.75
...
sce$celltype <- "other"
sce$CXCL13 <- "negative"
# define CD8 T cells for outlier samples
out_sce <- sce[,(sce$Description %in% c("E2","F2","D4"))]
out_sce[,which(t(assay(out_sce,"asinh"))[,"CD8a"] > 2.5 & t(assay(out_sce,"asinh"))[,"CD3"] > 1.75 ) ]$celltype <- "CD8_Tcell"
out_sce[,which(t(assay(out_sce,"asinh"))[,"CXCL13"] > 2.75 ) ]$CXCL13 <- "positive"

# define CD8 T cells for outlier samples
cur_sce <- sce[,!(sce$Description %in% c("E2","F2","D4"))]
cur_sce[,which(t(assay(cur_sce,"asinh"))[,"CD8a"] > 2 & t(assay(out_sce,"asinh"))[,"CD3"] > 1) ]$celltype <- "CD8_Tcell"
cur_sce[,which(t(assay(cur_sce,"asinh"))[,"CXCL13"] > 2 ) ]$CXCL13 <- "positive"
#merge samples back together
sce <- cbind(cur_sce,out_sce)
rm(cur_sce,out_sce)
...
names_ordered <- ex_markers[which(ex_markers != "CXCL13")]
...
```{r}
dittoScatterPlot(CD8_sce,x.var = "CXCL13",y.var = "PD-1",assay.x = "asinh",assay.y = "asinh")
...
ex_markers_noCXCL13 <- ex_markers[c(2,4,5,6,7,8)]

g <- buildSNNGraph(CD8_sce,subset.row = ex_markers_noCXCL13, k=20, type = "jaccard",assay.type = "scaled_0.99")
...
dittoHeatmap(CD8_sce, genes = ex_markers,annot.by =  c("clusters", "Description"),assay = "scaled_0.99",
             heatmap.colors = colorRampPalette(c("dark blue", "white", "dark red"))(100),
            breaks = seq(-3,3, length.out = 101))
...
## exhausted cluster fraction correlation with CXCL13+ fraction in protein data only
...
## mean expresion of exhaustion markers per image

```{r}
mean_sce <- aggregateAcrossCells(CD8_sce[ex_markers,],
                                ids = CD8_sce$Description,
                                statistics = "mean")
assay(mean_sce,"asinh") <- asinh(counts(mean_sce))

dittoHeatmap(mean_sce,genes = ex_markers,annot.by = c("Description","dysfunction_score"),assay = "asinh", cluster_cols = TRUE)
```
