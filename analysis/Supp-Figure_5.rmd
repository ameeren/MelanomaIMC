...
```{r, Supp_Fig_5C_responder_nonresponder_tcf7pd1, fig.height=7.3, fig.width=11.2, dev=("pdf")}
...
# per patient 
cur_dat <- cur_dat %>%
  group_by(PatientID, celltype2, TCF7_PD1, Status_at_3m) %>%
  summarise(n_per_mm2 = mean(n_per_mm2))
  
stat.test <- cur_dat %>%
  filter(TCF7_PD1 %in% c("TCF7+_PD1+", "TCF7+_PD1-", "TCF7-_PD1+")) %>%
...
ggplot(cur_dat[cur_dat$TCF7_PD1 %in% c("TCF7+_PD1+", "TCF7+_PD1-", "TCF7-_PD1+"),], aes(x=Status_at_3m, y=log10(n_per_mm2+1))) + 
  ...
  facet_wrap(~celltype2, scales = "free", ncol=3, nrow=2)
```
...
```{r Supp_Fig_5D_chemokines_tcfpd1_correlation, fig.height=6.2, fig.width=10.1, dev=("pdf")}
...
```{r Supp_Fig_5D_legend_heatmap, dev="pdf", fig.height=3, fig.width=2}
