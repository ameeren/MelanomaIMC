...
ggplot(celltypes, aes(x=celltype, y=fraction, fill=dysfunction_score)) +
  geom_boxplot(alpha=.5, lwd = 1, outlier.shape = NA) +
...
```{r Fig_5B_bcell_groups_fraction, dev=("pdf"), fig.width=10, fig.height=3}
...
# Bcell Patches and CXCL13 Expression
...
```{r Fig_5E_cd8_tcf7_pd1_bcells, dev=("pdf"), fig.width=9, fig.height=4}
celltypes <- data.frame(colData(sce_prot)) %>%
  mutate(mmLocationPunch = paste(MM_location, Location, sep = "_")) %>%
...
ggplot(celltypes, aes(x=TCF7_PD1, y=fraction, fill=bcell_patch_score)) +
  geom_boxplot(alpha=1, lwd = 0.5, outlier.size = 0.5) +
  #geom_jitter(alpha=0.5, position = position_jitterdodge(dodge.width = 0.75,jitter.width = 0.1), size = 1, 
              #aes(group = bcell_patch_score)) + 
  #scale_color_discrete(guide = FALSE) +
  stat_compare_means(aes(group = bcell_patch_score), label = "p.signif", size=6, label.y = 1) +
  theme_bw() +
...
```{r Fig_3B_clusters_and_communities, fig.width=6, fig.height=6, dev=c('pdf')}
...
```{r Supp_Fig_5X_milieu_presence_tcells, fig.height=6, fig.width=6, dev=("pdf")}
...
sum <- left_join(cur_dat, milieu_area)
