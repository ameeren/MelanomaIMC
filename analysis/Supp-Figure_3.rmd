...
```{r Supp_Fig_3X_tumor_profile_tcell, fig.height=8, fig.width=10, dev=c('pdf')}
...
```{r Supp_Fig_3X_tumor_profile_tcell_legend, fig.height=2, fig.width=2, dev=c('pdf')}
grid.arrange(leg)
```
...
```{r Supp_Fig_3X_chemokines_tumor_interaction, dev=("pdf"), fig.width=9, fig.height=8}
...
```{r Supp_Fig_3X_expressor_cd8_grouping, fig.width=14, fig.height=5, dev=("pdf")}
targets <- metadata(sce_rna)$chemokines_morethan600_withcontrol
...
ggplot(frac, aes(x=expressor, y = fraction, fill = Tcell_density_score_image)) + 
  ...
  guides(fill=guide_legend(title="CD8 Density")) +
  xlab("") + 
  ylab("Fractions")  +
