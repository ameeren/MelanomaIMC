...
```{r boxplot_HLADR_B2M, dev=('pdf'), fig.width=11, fig.height=6}
cur_sce <- sce_rna[,sce_rna$cellID %in% cbind(tumor_chemokine, control_tumor)]
...
```{r UMAP_chemokine_stand_tumor, dev=('pdf'), fig.width=8, fig.height=6}
...
```{r boxplot_HLADR_B2M_no_shared, dev=('pdf'), fig.width=11, fig.height=6}
cur_sce <- sce_rna[,sce_rna$cellID %in% cbind(tumor_chemokine, control_tumor)]
...
````{r dev=('pdf'), fig.width=8, fig.height=6}
...
## Check HLADR levels of neighboring cells (maybe HLADR in tumor comes from neighbors)

```{r}
# sample same amount of control tumor cells
cur_sce <- sce_rna[,sce_rna$cellID %in% c(standalone_neighbours, standalone_tumor)]

# add info if the tumor cell is producing a chemokine or not (STANDALONE)
cur_df <- data.frame(colData(cur_sce))
cur_df$tumor_neighbor <- ifelse(cur_df$cellID %in% standalone_neighbours, "neighbouring cell", "tumor cell")
colData(cur_sce)$tumor_neighbor <- cur_df$tumor_neighbor

neighbor_dat <- data.frame(t(assay(cur_sce["HLADR", cur_sce$Location != "CTRL"], "asinh")))
neighbor_dat$cellID <- rownames(neighbor_dat)
neighbor_dat <- left_join(neighbor_dat, data.frame(colData(cur_sce))[,c("cellID", "tumor_neighbor")])

a <- ggplot(neighbor_dat, aes(x=tumor_neighbor, y=HLADR)) + 
  geom_boxplot() +
  theme(text = element_text(size=20)) +
  xlab("Chemokine Production in Tumor Cells") + 
  ylab("Mean HLA-DR in Cells (asinh)")

# sample same amount of control tumor cells
cur_sce <- sce_rna[,sce_rna$cellID %in% c(control_neighbours, control_tumor)]

# add info if the tumor cell is producing a chemokine or not (STANDALONE)
cur_df <- data.frame(colData(cur_sce))
cur_df$tumor_neighbor <- ifelse(cur_df$cellID %in% control_neighbours, "neighbouring cell", "tumor cell")
colData(cur_sce)$tumor_neighbor <- cur_df$tumor_neighbor
...
b <- ggplot(neighbor_dat, aes(x=tumor_neighbor, y=HLADR)) + 
  geom_boxplot() +
  theme(text = element_text(size=20)) +
  xlab("No Production in Tumor Cells") + 
  ylab("Mean HLA-DR in cells (asinh)")

grid.arrange(a,b,ncol=2)
```

## Check B2M levels of neighboring cells
...
neighbor_dat <- data.frame(t(assay(cur_sce["B2M", cur_sce$Location != "CTRL"], "asinh")))
neighbor_dat$cellID <- rownames(neighbor_dat)
neighbor_dat <- left_join(neighbor_dat, data.frame(colData(cur_sce))[,c("cellID", "tumor_neighbor")])

a <- ggplot(neighbor_dat, aes(x=tumor_neighbor, y=B2M)) + 
  geom_boxplot() +
  theme(text = element_text(size=20)) +
  xlab("Chemokine Production in Tumor Cells") + 
  ylab("Mean B2M in Cells (asinh)")
...
b <- ggplot(neighbor_dat, aes(x=tumor_neighbor, y=B2M)) + 
  geom_boxplot() +
  theme(text = element_text(size=20)) +
  xlab("No Production in Tumor Cells") + 
  ylab("Mean B2M in cells (asinh)")
...
## Marker Heat Map of Neighbouring Cells
