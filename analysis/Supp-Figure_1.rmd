...
# Introduction
This script generates plots for Supplementary Figure 1. It contains data that is not stored in the SCE format. 
...
## Compare PPIB strenght in single-stains and multiple stains

```{r Supp_Fig_1X_single_vs_multi_stain, fig.width=8, fig.height=7, dev=c('pdf')}
singleStain <- cells_panel_12plex_sub[cells_panel_12plex_sub$Target == "T2",]
singleStain$info <- "Single Target"
multipleStain <- dd[dd$Target == "T2" & dd$stain == "- Antibodies", ]
multipleStain$info <- "Multiple Targets"

combined <- rbind(singleStain[,c("counts_asinh", "replicate", "info")], 
                  multipleStain[,c("counts_asinh", "replicate", "info")])

stats <- combined %>%
  anova_test(counts_asinh ~ info) %>%
  adjust_pvalue() %>%
  add_significance()

ggplot(data=combined, aes(x=info, y=counts_asinh, col=replicate)) + 
  geom_boxplot(outlier.shape = NA) +
  geom_quasirandom(size=.5, alpha=.1, dodge.width = 0.75) +
  ylab("PPIB Expression Channel 2 (asinh)") +
  xlab("Staining ") + 
  theme_bw() +
...
```{r Supp_Fig_1F_number_of_samples, fig.height=6, fig.width=17, dev=c('pdf')}
...
## Correlation across the two data sets

```{r Supp_Fig_1G_pearson_correlation__prot_data, fig.width=9, fig.height=9, dev=c('pdf')}
