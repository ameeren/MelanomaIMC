...
sapply(list.files("~/bbvolume/bbvolume/server_homes/daniels/Git/imcRtools/R/", full.names = TRUE), source)
library(LSD)
...
# change colData name of from celltype_layer1 to celltype
sce$celltype <- sce$celltypes_layer1
sce$celltypes_layer1 <- NULL
```
...
## phenograph clustering the sub-sampled data.

the problem with phenograph is that you can only defined k and even with very low k for some super-classes you still get too many clusters! therefore FlowSom might be preferential to exactly define how many clusters one wants per
super-class. FlowSom code further below.

```{r sublcustering using PhenoGraph sub-sampled, eval = FALSE, echo=FALSE}
# subsample data
sce_samp <- sce[,sample(c(1:length(colnames(sce))),size = 100000)]

metadata(sce_samp) <- as.list(metadata(sce_samp))

# vector for clustering
pg_clustering <- vector(length = ncol(sce_samp))


# Perform phenograph clustering on selected markers
# Make sure that names of marker list matches celltype names
set.seed(12345)
for(i in names(marker_list)){
  cur_sce <- sce_samp[,sce_samp$celltype == i]
  
  # Phenograph community detection on nearest neighbour graph. Potentially one needs to play with different transformations of the data...
  communities <- Rphenograph(t(as.matrix(assay(cur_sce, "logcounts"))[marker_list[[i]],]), 
                             k = 30)
  clusters <- factor(membership(communities[[2]]))
  
  pg_clustering[sce_samp$celltype == i] <- clusters
}

# Save in SCE object
colData(sce_samp)$gating_clustering_PG <- as.factor(pg_clustering)
```

## visulalize clustering results for PhenoGraph

```{r mean expression per celltype, fig.width= 20, fig.height=15, eval=FALSE, echo=FALSE}
# Aggregate the counts
mean_sce_samp <- calculateSummary(sce_samp, split_by = c("celltype","gating_clustering_PG"), 
                             exprs_values = "counts")

# Exclude DNA and Histone
mean_sce_samp <- mean_sce_samp[!grepl("DNA|Histone", rownames(mean_sce_samp)),]

# Transform and scale
assay(mean_sce_samp, "arcsinh") <- asinh(assay(mean_sce_samp, "meanCounts")) 
assay(mean_sce_samp, "arcsinh_scaled") <- t(scale(t(asinh(assay(mean_sce_samp, "meanCounts")))))

# Select informative markers
markers <- c("CD45RA", "CD45RO", "Ecad", "CD303", "MPO", "CD3", "CD7", "CD38", "HLADR", "CD20")

# Linear scale 
plotHeatmap(mean_sce_samp, exprs_values = "arcsinh", 
            colour_columns_by = c("celltype"), 
            color = viridis(100), labels_col = paste0(mean_sce_samp$celltype, "_", mean_sce_samp$gating_clustering_PG),
            show_colnames = TRUE, annotation_legend = FALSE, borders_color = NA)

# Scaled
plotHeatmap(mean_sce_samp, exprs_values = "arcsinh_scaled", 
            colour_columns_by = c("celltype"), 
            labels_col = paste0(mean_sce_samp$celltype, "_", mean_sce_samp$gating_clustering_PG),
...
for(i in c("pDC","Neutrophil","BnTcell","Bcell","Tregulatory","Stroma")){
  cur_sce <- sce[,sce$celltype == i]
...
annotations <- sce$gating_clustering_FS
...
## from the subclustering we now update the celltype

```{r re-assign celltypes, eval=FALSE}
