...
survdat <- read.csv("data/200323_TMA_256_Hot Cold_Clinical Data_Updated Response Data_For Collaborators_latest updated_Mar_2020_for_Coxph_modeling.csv",sep = ";")
```
...
res <- coxph(Surv(survdat$Time_to_.progression_or_last_PET,survdat$censoring_progression) ~
               IHC.T.cell.scoring, data = survdat)

td_res = broom::tidy(res, exponentiate = TRUE, conf.int = TRUE)
...
td_res = tidy(res, exponentiate = TRUE, conf.int = TRUE)
...
## Kaplan Meier plot for Pre-Treatments

```{r Kaplan_Meier_for pre-treatment, fig.height=8, fig.width=12}
# add binary grouping for number of pre-treatments
survdat$treatment_naive <- ifelse(survdat$Number.of.treatments.before.surgery == 0, TRUE, FALSE)

# generate survival object
Surv_RFS <- Surv(time = survdat$Time_to_death_or_last_PET,
                 event = survdat$censoring_death)

KM_all <- survfit(Surv_RFS ~ 1)

KM_groups <- survfit(Surv_RFS ~ survdat$treatment_naive)

cols <- c("red","blue")
names(cols) <- c("non-naive","treatment naive")

#survdat[survdat$Number.of.treatments.before.surgery == 0,]$censoring_death
...
## Kaplan Meier plot for treatment-naive patients using cytotoxic Tcell fraction

```{r Kaplan_Meier_naive_Tcell_fraction, fig.height=8, fig.width=12}
# subset survdat for treatment-naive patients
survdat_sub <- survdat[survdat$treatment_naive == FALSE,]

# generate survival object
Surv_RFS <- Surv(time = survdat_sub$Time_to_.progression_or_last_PET,
                 event = survdat_sub$censoring_progression)

KM_all <- survfit(Surv_RFS ~ 1)

KM_groups <- survfit(Surv_RFS ~ survdat_sub$Tcell_fraction_per_BlockID)


cols = brewer.pal(3,"Dark2")
names(cols) <- levels(survdat_sub$Tcell_fraction_per_BlockID)

plot(KM_groups,mark.time = T,col = cols,conf.int = F, lwd=5, 
     cex.axis=1.5, cex.lab = 1.5)
par(new=TRUE)
plot(KM_all,mark.time = T,col = "black",conf.int = T, 
     xlab = "days", ylab = "RFS (%)", lwd=2,
     cex.axis=1.5, cex.lab = 1.5)
legend("topright",1,names(cols),fill=cols,cex = 2)
```

## Coxph modeling for treatment-naive patients 

```{r}
# relevel survdat Tcell_fraction_per_BlockID
survdat_sub$Tcell_fraction_per_BlockID <- factor(survdat_sub$Tcell_fraction_per_BlockID, 
                                                 levels = c("low", "med", "high"))

# coxph model
res <- coxph(Surv(survdat_sub$Time_to_.progression_or_last_PET,survdat_sub$censoring_progression) ~
               Tcell_fraction_per_BlockID, data = survdat_sub)
...
ggplot(td_res, aes(x = term, y = estimate, color = p.value < 0.05)) +
  ...
  scale_color_manual(values = c('lightblue','red'))+
  geom_hline(yintercept = 1, color = 'blue')  +
  scale_x_discrete(labels = c("Tcell_fraction_per_BlockIDhigh" = "high fraction",
                              "Tcell_fraction_per_BlockIDmed" = "medium fraction")) +
  xlab("cytotoxic Tcell fraction per BlockID") +
