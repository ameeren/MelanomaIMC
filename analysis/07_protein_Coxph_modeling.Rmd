...
## coxph modeling for T cell score

```{r Hazard ratio for T cell score}
Tcells_per_BlockID <- data.frame(BlockID = sce$BlockID,
                                      Tcell_score_per_BlockID = sce$Tcell_score_per_BlockID)

Tcells_per_BlockID <- unique(Tcells_per_BlockID)

Tcell_block <- Tcells_per_BlockID$Tcell_score_per_BlockID
names(Tcell_block) <- Tcells_per_BlockID$BlockID


survdat$Tcells_per_BlockID <- Tcell_block[survdat$Block.ID]

# add factor for T cell scoring
survdat$Tcells_per_BlockID <- factor(survdat$Tcells_per_BlockID,levels = c("high","med","low","absent"))
  
res <- coxph(Surv(survdat$Time_to_.progression_or_last_PET,survdat$censoring_progression) ~ Tcells_per_BlockID,data = survdat)
...
## Kaplan Meier plot for Death split by Tcell_score

```{r Kaplan Meier Tcell score survival for BlockID, fig.height=6, fig.width=8}
# generate survival object
Surv_RFS <- Surv(survdat$Time_to_death_or_last_PET,survdat$censoring_death)

KM_all <- survfit(Surv_RFS ~ 1)

KM_groups <- survfit(Surv_RFS ~ survdat$Tcells_per_BlockID)

cols = brewer.pal(4,"Dark2")
names(cols) <- levels(survdat$Tcells_per_BlockID)
...
## Kaplan Meier plot for treatment naive BlockIDs for Death split by Tcell_score

```{r Kaplan Meier Tcell score survival for untreated, fig.height=6, fig.width=8}
# generate survival object
...
KM_groups <- survfit(Surv_RFS ~ survdat[which(survdat$Number.of.treatments.before.surgery == 0),]$Tcells_per_BlockID)
...
```
## Kaplan Meier plot for relapsesplit by Tcell score

```{r Kaplan Meier Tcell score progression, fig.height=8, fig.width=10}
# generate survival object
...
plot(KM_groups,mark.time = T,col = cols,conf.int = F)
par(new=TRUE)
plot(KM_all,mark.time = T,col = "black",conf.int = T, xlab = "days", ylab = "RFS (%)")
...
```{r ggplot Kaplan Meier curve,eval=FALSE, fig.height=5, fig.width=10}
ggplot(tidy(KM_all), aes(time, estimate)) + geom_line() +
    geom_ribbon(aes(ymin=conf.low, ymax=conf.high), alpha=.25)+
  geom_line(data = tidy(KM_groups),aes(time,estimate, group=strata, color=strata))+
  ylim(0,1)+
  theme_bw()+
  labs(x="days",y="RFS (%)")
...
## Kaplan Meier plot for progression for MM_location
...
## coxph modeling for MM_location

```{r Hazard ratio for MM_location, fig.height=8, fig.width=12}

res <- coxph(Surv(survdat$Time_to_death_or_last_PET,survdat$censoring_death) ~ MM.location,data = survdat)
