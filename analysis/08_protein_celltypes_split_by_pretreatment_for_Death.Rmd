...
sce <- sce[,which(sce$treatment_group_before_surgery == "ICI")]
```



## Death analysis for samples with pre-treatment ICI
...
```{r Macrophage cluster fractions for Death, fig.height=10, fig.width=20, message=FALSE}
...
cur_df_long %>%
  filter(variable %in% c("H3K27me3","HLADR","S100","Sox9","pERK","CD36","p75","MITF","PDL1","bCatenin","Ki67","Collagen1","SOX10","pS6")) %>%
  ggplot()+
  geom_boxplot(aes(x = celltype,y=value, fill=Death))+
  facet_wrap(~variable)+
  theme(axis.text.x = element_text(angle = 90, vjust = 1))
```
