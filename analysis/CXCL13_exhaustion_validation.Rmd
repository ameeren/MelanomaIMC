...
```{r}
sce <- read_steinbock(path = "/media/dan/Data/Data/Melanoma_HotCold_paper/20210915_YTMA256_exhaustion_stain_DS/",return_as = "sce",graphs_folder = NULL, regionprops_folder = NULL)
...
```{r}
for (i  in unique(sce$Description)) {
  p <-dittoScatterPlot(sce[,which(sce$celltype == "CD8_Tcell" & sce$Description == i )],x.var = "CXCL13",y.var = "GITR",assay.x = "asinh",assay.y = "asinh")
  print(p)
}
...
```{r}
dittoScatterPlot(sce[,which(sce$celltype == "CD8_Tcell")],x.var = "CXCL13",y.var = "PD-1",assay.x = "asinh",assay.y = "asinh")

cur_sce <- sce[,sce$celltype == "CD8_Tcell"]
cur_sce[,which(t(assay(cur_sce,"asinh"))[,"PD-1"] > 1.3 & t(assay(cur_sce,"asinh"))[,"CXCL13"]< 2)]$celltype <- "PD-1+_CXCl13-_Tcell"
cur_sce[,which(t(assay(cur_sce,"asinh"))[,"PD-1"] > 1.3 & t(assay(cur_sce,"asinh"))[,"CXCL13"]> 2)]$celltype <- "PD-1+_CXCl13+_Tcell"
cur_sce[,which(t(assay(cur_sce,"asinh"))[,"PD-1"] < 1.3 & t(assay(cur_sce,"asinh"))[,"CXCL13"]> 2)]$celltype <- "PD-1-_CXCl13+_Tcell"

dittoScatterPlot(cur_sce,x.var = "CXCL13",y.var = "PD-1",assay.x = "asinh",assay.y = "asinh",color.var = "celltype")
...
cur_dat %>%
  ...
  ggplot()+
  geom_bar(aes(x=Description,y=frac, fill=celltype),stat = "identity")

cur_sce <- sce[,sce$celltype == "Tcell"]
cur_sce[,which(t(assay(cur_sce,"asinh"))[,"PD-1"] > 1.3 & t(assay(cur_sce,"asinh"))[,"CXCL13"]< 2)]$celltype <- "PD-1+_CXCl13-_Tcell"
cur_sce[,which(t(assay(cur_sce,"asinh"))[,"PD-1"] > 1.3 & t(assay(cur_sce,"asinh"))[,"CXCL13"]> 2)]$celltype <- "PD-1+_CXCl13+_Tcell"
cur_sce[,which(t(assay(cur_sce,"asinh"))[,"PD-1"] < 1.3 & t(assay(cur_sce,"asinh"))[,"CXCL13"]> 2)]$celltype <- "PD-1-_CXCl13+_Tcell"
...
```
## compare RNA and protein CXCL13 T cell fractions
...
rna_dat <- RNA_dat %>%
  filter(Location != "CTRL", Tcell_density_score_image == "high") %>%
  ...
  mutate(total_cellcount = sum(n),frac=n/total_cellcount) %>%
  filter(celltype == "CD8+ T cell_1") %>%
  pull(frac,Description)

# now we calculate the fractions for the protein dataset
prot_dat <- cur_dat %>%
  select(Description,celltype) %>%
  group_by(Description,celltype) %>%
  count(celltype) %>%
  ungroup() %>%
  group_by(Description) %>%
  mutate(total_cellcount = sum(n)) %>%
  filter(celltype %in% c("PD-1+_CXCl13+_Tcell","PD-1-_CXCl13+_Tcell")) %>%
  mutate(CD8_CXCL13 = sum(n),frac = CD8_CXCL13/total_cellcount) %>%
  select(Description,frac) %>%
  distinct() %>%
  pull(frac,Description)
  
rna_dat <- rna_dat[names(rna_dat) %in% names(prot_dat)]
all_dat <- data.frame(Description = names(rna_dat),
                      rna_frac = as.vector(rna_dat),
                      prot_frac = as.vector(prot_dat))

all_dat <- pivot_longer(all_dat,cols = c("rna_frac","prot_frac"))

library(ggpubr)
ggplot(all_dat, aes(x=prot_frac, y=rna_frac)) + 
  geom_point(size=3) + 
