...
# Introduction
This script generates plots for Figure 5 without any LN samples.
...
```{r Fig_5B_celltype_fractions_dysfunction, dev=("pdf"), fig.width=8.6, fig.height=5.35}
...
```{r Fig_5D_Bcell_patches_CXCL13_fractions, fig.height=6.2, fig.width=7.9, dev=c('pdf')}
...
example <- findPatch(sce_prot[,sce_prot$Description == "M3"], sce_prot[,colData(sce_prot)$celltype %in% c("B cell", "BnT cell")]$cellID, 
                    'cellID', 
                    'Center_X', 'Center_Y', 
                    'Description', 
                    distance = 15, 
...
```{r Fig_5F_cd8_tcf7_pd1_bcells, dev=("pdf"), fig.width=6.66, fig.height=3.16}
celltypes <- data.frame(colData(sce_prot)) %>%
  mutate(mmLocationPunch = paste(MM_location, Location, sep = "_")) %>%
...
```{r Fig_5F_zoom, dev=("pdf"), fig.width=4.1, fig.height=2}
...
```{r Fig_5G_milieu_presence_tcells, fig.height=4, fig.width=4.9, dev=("pdf")}
