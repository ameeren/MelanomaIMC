...
```{r Fig_3A_clusters_and_communities, fig.width=10, fig.height=10, dev=c('pdf')}
...
```{r Fig_3B_UMAP_kmeans_cluster_chemokines_abundance, dev=c('pdf')}
...
```{r Fig_3C_community_modules_chemokines_abundance_boxplot, fig.height=6.5, fig.width=16, dev=c('pdf')}
freqs_long %>%
...
```{r Fig_3D_UMAP_producing_cells, fig.height=10, fig.width=10, dev=c('pdf')}
...
for(i in c("cxcl13only_clust")){
  ...
  a <- dittoDimPlot(sce_sub,
                    ...
                    legend.show = TRUE,
                    opacity = 0.3,
                    max.color = "blue", min.color = "gray88",
                    main = NULL) +
    theme_minimal() +
...
```{r Fig_3E_UMAP_neighbouring_producing_cells, fig.height=10, fig.width=10,dev=c('pdf')}
# loop through all pure clusters 
