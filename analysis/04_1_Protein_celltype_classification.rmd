...
layer1_colours["BnTcell"] <- "goldenrod2"
layer1_colours["Bcell"] <- "green1"
layer1_colours["Macrophage"] <- "greenyellow"
layer1_colours["Neutrophil"] <- "blue1"
layer1_colours["Thelper"] <- "yellow"
layer1_colours["Tregulatory"] <- "sienna1"
...
dittoDimPlot(sce[,sce$layer_1_gated != "unlabelled"], var = "celltypes_layer1", reduction.use = "UMAP", size = 0.5, 
...
unlab_sce <- sce[,sce$layer_1_gated == "unlabelled"]
agr_sce <- aggregateAcrossCells(unlab_sce, ids = colData(unlab_sce)[,c("ImageNumber", "celltypes_layer1")], 
                                average = TRUE)
assay(agr_sce, "asinh") <- asinh(counts(agr_sce))

colnames(agr_sce) <- paste0(agr_sce$ImageNumber, "_", agr_sce$celltypes_layer1)

rownames(sce)
# Non-scaled
dittoHeatmap(agr_sce[cur_markers,], assay = "asinh",
            annot.by = c("celltypes_layer1"), 
            order.by = "celltypes_layer1", cluster_rows = FALSE,
            scale = "none", heatmap.colors = viridis(100), 
            annotation_colors = list(celltypes_layer1 = metadata(sce)$colour_vectors$layer_1))
...
## Create updated colour vector

Here, we will define a colour vector for the cell-types contained in layer 1 (without unlabelled).

```{r}
layer1_colours <- vector(length = length(unique(sce$celltypes_layer1)))
names(layer1_colours) <- unique(sce$celltypes_layer1)
...
# Save in SCE object
metadata(sce)$colour_vectors$celltypes <- layer1_colours
```

# Save RDS

```{r saveRDS}
saveRDS(sce, "data/sce_protein.rds")
...
write.csv(layer_1_dat, file = "data/layer_1_classification_protein.csv")
```
