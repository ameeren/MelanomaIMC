...
label.files <- list.files("data/protein/celltype_classifier/", full.names = TRUE)
...
```{r}
layer1_colours <- vector(length = length(unique(sce$celltype)))
names(layer1_colours) <- unique(sce$celltype)
...
# Save in SCE object
metadata(sce)$colour_vectors$celltype <- layer1_colours
```
...
saveRDS(layer_1_dat, "data/layer_1_classification_protein.rds")
```
