...
sub_sce_rna <- NULL
sub_sce_prot <- NULL
#sub_sce_rna <- sce_rna[,colData(sce_rna)$celltype %in% c("Tumor")]
#sub_sce_prot <- sce_prot[,colData(sce_prot)$celltype %in% c("Tumor")]
...
# Plot Cell Numbers
a <- ggplot(data = summary, aes(x=interaction("cells",summary$data_set),y=log10(cells))) + 
...
# Plot Cell Numbers
b <- ggplot(data = summary, aes(x=interaction("cells",summary$data_set),y=log10(mean_cell_area))) + 
  geom_boxplot(aes(fill=data_set)) + 
  geom_point(aes(group=data_set), alpha =0.3) + 
  geom_line(aes(group=Description),alpha=0.2) + 
  scale_x_discrete(labels = rep("mean_cell_area", each = 2))

c <- ggplot(data = summary, aes(x=interaction("cells",summary$data_set),y=log10(image_area))) + 
  geom_boxplot(aes(fill=data_set)) + 
  geom_point(aes(group=data_set), alpha =0.3) + 
  geom_line(aes(group=Description),alpha=0.2) + 
...
## Compare cell density in both data sets (cells per 1000µm2)
