...
```{r Fig_1C_Scaled_Heatmap_RNA, fig.width=10, fig.height=11, dev=c('pdf')}
...
# Figure 1D - Plots for Protein TMA

## Barplot containing Cell Types per Image

```{r Fig_1D_Protein_Barplot, fig.width=13, fig.height=8, dev=c('pdf')}
...
```{r Fig_1C_Scaled_Heatmap_Protein, fig.width=10, fig.height=11, dev=c('pdf')}
# add marker expression to cells
...
```{r Fig_1E_Celltype_Correlation, dev=c('pdf'), fig.height=8, fig.width=9}
...
## Immune Cell Correlation in Protein Set

```{r Fig_1F_Immune_Correlation, dev=c('pdf'), fig.height=7, fig.width=7, eval=F}
cur_prot <- data.frame(colData(sce_prot))
...
# Correlation Plot - shared celltypes
immune <- c("Macrophage", "Neutrophil", "Tcytotoxic", "Bcell", "BnTcell", "pDC", "Thelper", "Tregulatory")

# correlation
cor <- cor(prot_sum[,immune], prot_sum[,immune], method = "pearson")
...
```{r Fig_4A_example_images_B2Mexpression, dev=("pdf")}
# subset masks
