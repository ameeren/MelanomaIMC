...
```{r Supp_Fig_4X_Scaled_Heatmap_Protein, fig.width=11, fig.height=11, dev=c('pdf')}
...
ha <- HeatmapAnnotation("Number of Cells" = anno_barplot(ifelse(as.numeric(dat_sum[2,])>20000, 20100, as.numeric(dat_sum[2,])),
                                                         height = unit(2,"cm"),
                                                         ylim = range(0,20000),
                                                         gp = gpar(fill = ifelse(as.numeric(dat_sum[2,]) > 20000, "white", "black"),
                                                                   col = "white")),
                        "Numbers" = anno_text(round(as.numeric(dat_sum[2,])), 
                                                   ...
                                                   just = "center", 
                                                   location = 0.5,
                                                   gp = gpar(fontsize=8,col = ifelse(as.numeric(dat_sum[2,]) > 20000, "red", "black"))))
...
```{r Supp_Fig_4X_heatmap_modules_infiltration, fig.height=8, fig.width=14, dev=c('pdf')}
...
```{r Supp_Fig_4X_legend, dev=c('pdf'), fig.width=3, fig.height=1}
# manual legends
...
lgd2 = Legend(col_fun = colorRamp2(c(0:max(m)), colors = col_fun), 
              at = seq(0, max(m)+2, by=5), title = "Detected Patches", direction = "horizontal", grid_width = unit(2, "cm"))
...
```{r Supp_Fig_4X_tumor_profile_per_image, fig.height=8, fig.width=7, dev=c('pdf')}
...
# plot 
p <- ggplot(comb, aes(x=dysfunction_score, y=asinh, fill=dysfunction_score)) + 
...
```{r Supp_Fig_4X_tumor_profile_per_image_legend, fig.height=2, fig.width=2, dev=c('pdf')}
grid.arrange(leg)
```

# Manual S100 cut-off on tumor cells
...
```{r Supp_Fig_4X_S100_manual, fig.height=5, fig.width=5, dev=c('pdf')}
...
ggplot(s100, aes(x=dysfunction_score, y=fraction, fill =dysfunction_score)) + 
  geom_boxplot(alpha=0.2, lwd=1.5) + 
  geom_quasirandom(aes(col=dysfunction_score), size=3) +
  stat_compare_means(comparisons = list(c("high dysfunction", "low dysfunction")), size=6) + 
  xlab("") + 
