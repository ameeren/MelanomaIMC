...
sce <- readRDS("/media/dan/Data/data_for_analysis/sce_protein.rds")
```

```{r all cells heatmap, fig.width=10, fig.height=8}
set.seed(2345)
...
cur_sce <- cur_sce[,sample(colnames(cur_sce),size = 2000,replace =FALSE)]
...
```{r labelled cells heatmap, fig.width=10, fig.height=8}
set.seed(2345)
...
```{r  RNA data all cells heatmap, fig.width=10, fig.height=8}
sce <- readRDS("/media/dan/Data/data_for_analysis/sce_RNA.rds")
...
```{r RNA data labelled cells heatmap, fig.width=10, fig.height=8}
set.seed(2345)
