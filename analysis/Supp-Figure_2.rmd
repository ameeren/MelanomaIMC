...
# Introduction
This script generates plots for Supplementary Figure 2. It contains data that is not stored in the SCE format. 
...
```{r Supp_Fig_2E_heatmap_unspecific_binding, dev=c('pdf')}
...
```{r Supp_Fig_2D_corrected signal, dev=("pdf"), fig.width=8, fig.height=5}
...
```{r Supp_Fig_2A_merged_comparison_per_channel, fig.width=10, fig.height=8, dev=c('pdf')}
# subset
...
```{r Supp_Fig_2C_overnight_Ab_incubation, fig.width=10, fig.height=8, dev=c('pdf')}
...
```{r Supp_Fig_2B_correlation_with_RNAseq, fig.width=8, fig.height=7, dev=c('pdf')}
# add gene name
...
cor(sum$expression_RNAScope, log2(sum$nTPM), method = c("pearson"))
```
