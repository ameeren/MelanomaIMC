...
```{r load data}
sce = readRDS(file = "/Volumes/thoch/Git/melanoma_publication_old_data/data/sce_RNA.rds")
```

## load the segmentation masks and the pixel-level information (IMC images)

```{r}
all_mask <- loadImages(x = "/Volumes/thoch/Data/Analysis/melanoma_cohort_new_cp_pipeline/rna/cpout/",pattern = "ilastik_s2_Probabilities_equalized_cellmask.tiff")
all_pixel <- loadImages(x = "/Volumes/thoch/Data/Analysis/melanoma_cohort_new_cp_pipeline/rna/tiffs/",pattern = "_full.tiff")
```
...
image_mat <- as.data.frame(read.csv(file = "/Volumes/thoch/Data/Analysis/melanoma_cohort_new_cp_pipeline/rna/cpout/Image.csv",stringsAsFactors = FALSE))
...
```{r add channel names}
panel_meta <- read.csv(file = "/Volumes/server_homes/thoch/Data/Analysis/melanoma_cohort/processing/rna/v3/config/melanoma_1.06_rna.csv", sep= ",",  stringsAsFactors = FALSE )
